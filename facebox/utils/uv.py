...
from facebox.Sim3DR.Sim3DR import rasterize
from facebox.utils.functions import plot_image
from facebox.utils.io import _load
from facebox.utils.tddfa_util import _to_ctype
...
def nearest_interpolate(img, x, y):
    """
    Notes:
        This function is not present in the original repo.
            In our case the interpolation where playing major role.
    """
    x_rounded = np.round(x).astype(np.int32)
...
def uv_tex(img,
    ...
    if img.dtype == np.uint16:
        ...
        raise TypeError('Image type not supported. (only np.uint8 and np.uint16 are allowed)')

    res_lst = []
    for ver_ in ver_lst:
        ver = _to_ctype(ver_.T)  # transpose to m x 3
        if inter == 'bilinear':
            colors = bilinear_interpolate(img, ver[:, 0], ver[:, 1]) / max_val
        elif inter == 'nearest':
            colors = nearest_interpolate(img, ver[:, 0], ver[:, 1]) / max_val
        ...
        if yaw >= 10:
            midpoint = uv_w // 2
            right_part = res[:, midpoint:, :]

            # Mirror the right part to create the left part
            left_part = np.flip(right_part, axis=1)

            # Combine the left and right parts to form the new image
            res = np.concatenate((left_part, right_part), axis=1)

        elif yaw <= -10:
            midpoint = uv_w // 2
            left_part = res[:, :midpoint, :]
